 ...
 *   Bit5~4：出口选择，00为出口1, 01为出口2，10为出口3，11为出口4，Reset值为00\n
 *   Bit6：结束位中断允许，高有效，Reset值为0\n
 *   Bit7：IIC中断允许，高有效，Reset值为0\n
 *
 * - IICSTA：可读可写\n
 *   Bit3：起始位标志，当检查到起始位，则自动置1，软件读到后应清零，否则无法检测到下一次起始位，Reset值为0\n
 *   Bit4：输出ACK设置，0和1对应信号线上的低电平和上拉。，Reset值为0\n
 *   Bit5：输入的ACK或NACK，0和1对应信号线上检测到的低电平和高电平，Reset值为0\n
 ...
 *
 * - IICBAUD：波特率寄存器，Reset值为x，用之前需要初始化\n
 *   波特率计算公式：波特率 = Freq_sys / ((IICBAUD+1) *4)\n
 *   波特率寄存器只有在主机时才有用。\n
 *   波特率寄存器为只写寄存器\n
 *
 * \section iic2 IICBUF：
 * - 做主机时，写IICBUF会启动一次通信，写什么值就会发什么值出去，当普通pending出现，则表示发送完毕。此时可检查确认位（ACK， IICSTA[5]），以了解从机是否接收到。\n
